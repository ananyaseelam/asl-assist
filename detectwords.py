...
def detect_words():
    ...
    while True:
        ...
        # cv2.imshow('asl_assist',  cv2.resize(
        ...
        cv2.rectangle(image_np_with_detections, (0, 40),
                      (width, 80), (0, 0, 0), -1)
        cv2.putText(image_np_with_detections, ' '.join(sentence), (3, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

        # full_image = np.concatenate(
        #     (image_np, image_np_with_detections), axis=0)
        # # Show the text on the screen
        cv2.imshow('asl assist', image_np_with_detections)
